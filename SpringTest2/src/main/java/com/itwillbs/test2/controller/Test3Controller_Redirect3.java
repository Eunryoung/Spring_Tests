...
//		return "redirect:/redirectServlet";
		...
		//	  리다이렉트 시에는 객체가 넘어가지 않고 문자열로 넘어감
//	}
	...
	// ================================================================================
	/*
	 * [ 리다이렉트 방식 포워딩을 매핑하는 메서드 내에서 파라미터 데이터에 접근하는 방법 ]
	 * 1) HttpServletRequest 객체를 활용한 기존 방법(스프링에서 사용 X)
	 * 	  => 컨트롤러 매핑 메서드 정의시 HttpServletRequest 객체 타입 파라미터 선언 후
...
//	public String redirectServlet(String name, int age2) {
		...
		// => Integer.parseInt 안함(object->int)
		...
		// 	  (java.lang.IllegalStateException: Optional int parameter 'age2' is present 
...
//	public String redirectServlet(PersonVO person, char gender) {
		...
		// => 이 때., 데이터 저장과정에서 Setter(setXXX() 메서드가 자동으로 호출됨)
		// => 단, 멤버변수명과 파라미터 명이 일치하지 않으면 데이터 저장불가!
...
//		System.out.println("이름(Person) : " + person.getName() + ", 나이(Person) : " + person.getAge());
		...
		// e.g. PersonVO2에 gender 있어도 알아서 탐색해서 찾아줌
