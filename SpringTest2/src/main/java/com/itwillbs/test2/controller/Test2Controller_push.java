...
public class Test2Controller_push {
	...
	// 1번 방법) 매핑 메서드 파라미터를 통해 필요한 객체(HttpServletRequest) 주입 받기
//	@GetMapping("push") // 매핑 주소 지정시 
//	public String push(HttpServletRequest request) {
//		// 매핑 과정에서 매핑 메서드(push()) 호출 시점에 자동으로 HttpServletRequest 객체 전달(주입) 됨
//		// => JSP의 MVC 패턴에서 서블릿 클래스(XXXFrontController) 정의 시
...
//	}
	...
	// 2번 방법) 매핑 메서드 파라미터를 통해 Model 객체 주입 받기
	@GetMapping("push") // 매핑 주소 지정 시 
	public String push(Model model) {
		// 매핑 과정에서 매핑 메서드(push()) 호출 시점에 자동으로 Model 객체 전달(주입) 됨
