...
// => @Controller 어노테이션을 적용하면 스프링에서 컨트롤러 역할 클래스로 인식(매핑에 활용됨)
@Controller
...
//	@RequestMapping(value = "/main", method = RequestMethod.GET) // method 안의 내용이 ctrl+space해도 자동완성 안되므로 아래에 메서드 형태만 먼저 만들면 자동완성 된다!
...
//	}
	...
	// "/main" 서블릿 주소 요청을 POST 방식 요청으로 변경처리(405 오류 발생) -> 주소창에 /main 쳤으니 GET 방식 요청!(POST 방식은 폼태그로 적용시에만)
...
//	}
	...
	// value, method 명시 생략하면 적는 값이 value 값으로. 방식은 GET방식이 기본
	@RequestMapping("/main") // GET 방식 요청 매핑
	public String requestMain() {
		return "main";
	}
	...
	// => @GetMapping, @PostMapping, @PutMapping, @DeleteMapping 존재
...
//		System.out.println("GET 방식 /test2");
//		return "test/test2";
//	}
//	
//	@PostMapping("/test2")
//	public String test2_post() {
//		System.out.println("POST 방식 /test2");
//		return "test/test2";
//	}
	...
	// => value 속성에 복수개의 경로 매핑도 가능(쓸 일은 잘 없다)
	@RequestMapping(value = "/test2", method = {RequestMethod.GET, RequestMethod.POST}) // method 속성에 중괄호 적으면 자동완성 사용 가능!
	public String test2() {
