...
//			pstmt.setInt(11, 1); // 회원 상태(기본값 1로 처리 = 정상 회원)
...
//			isCorrectUser = rs.next();
		...
		return isCorrectUser; // MemberLoginProService로 이동
	}
	...
	public MemberBean selectMember(String id) {
		...
		try {
			...
			if(rs.next()) { // 조회 결과가 존재할 경우
				...
				member.setReg_date(rs.getDate("reg_date")); // 시간 포함 시 rs.getTimestamp() 메서드 사용
//				System.out.println(member); // 확인용 출력문
			}
			// => 3, 4단계 블록으로 묶어 try/catch 처리!
		} catch (SQLException e) {
